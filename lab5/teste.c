...
int main()
{   
    char inst[] = "jal x16, -1025738";
    InstData data;
    get_inst_data(inst, &data);
    int intinst = data.opcode;


    if (data.type == R) intinst |= (data.rd << 7) | (data.funct3 << 12) | (data.rs1 << 15) | (data.rs2 << 20) | (data.funct7 << 25);

    else if (data.type == I) intinst |= (data.rd << 7) | (data.funct3 << 12) | (data.rs1 << 15) | (data.imm << 20);

    else if (data.type == S) {
        int maskl = (1 << 5) - 1;
        int lowpart = data.imm & maskl;
        int maskh = ((1 << 12) - 1) - maskl;
        int highpart = (data.imm & maskh) >> 5;
        
        intinst |=  (lowpart << 7) | (data.funct3 << 12) | (data.rs1 << 15) | (data.rs2 << 20) | (highpart << 25);

    } else if (data.type == B) {
        int eleventhbit = (data.imm & 0x800) >> 11;
        int lowpart = (data.imm & 0b11110) | eleventhbit;
        int twelventhbit = (data.imm >> 12) & 0b1;
        int highpart = ((data.imm >> 5) & 0b111111) | (twelventhbit << 6);

        intinst |=  (lowpart << 7) | (data.funct3 << 12) | (data.rs1 << 15) | (data.rs2 << 20) | (highpart << 25);

    } else if (data.type == U) {

        intinst |= (data.rd << 7) | (data.imm << 12);

    } else if (data.type == J) {
        int imm19_12 = (data.imm >> 12) & (0xff);
        int imm11 = (data.imm >> 11) & (0x1);
        int imm10_1 = (data.imm >> 1) & (0x3ff);
        int imm20 = (data.imm >> 20) & (0x1);
        int number = 0;
        number |= (imm19_12 << 12); 
        number |= (imm11 << 20);
        number |= (imm10_1 << 21); 
        ...
        intinst |= (data.rd << 7) | (number);
    }

    char hex[11];
    hex_code(intinst, hex);
